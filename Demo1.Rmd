...
- Data scientists spend 80% of their time cleaning data rather than creating insights. As a result they only spend 20% of their time creating insights, the rest wrangling data. 

- This document demonstrates variety of processes used to transform raw data into more readily used formats.
...
```{r load}
# Collection of R packages designed for data science (data import, tidying, manipulation, visualisation, and programming)
library(tidyverse)
# Lubridate is an R package that makes it easier to work with dates and times.
library(lubridate)
```

### Preventing  Biofouling on Water Quality Instruments 

- Every fifth observation in turbidity in both sites are anomalous as a result of the wiper on the optical turbidity sensor operating every five minutes

- During our data cleansing process we  discarded these wiper anomalies in turbidity series from both sites.
...
```{r}
#  Discarded  wiper anomalies in turbidity series from both sites
data_turb_up <- waq_up %>% 
...
### Summary of quality flags of NEON data

- The NEON data  undergo an automated water-quality assuring process as a part of the rigorous NEON protocols, and are given a quality flag for each observation. 
...
* **fDOMTempQF** - corrected for temperature 
...
- Aggregated data such that timestamps match across two different datasets
...
# `thicken` creates evenly spaced intervals to which observations can be aggregated. pad subsequently checks for missing observations and inserts records where needed. 
# Take the datetime variable in a data frame and map this to a variable of a higher interval. 
...
- The quality flags are available in NEON data are not necessarily technical anomalies. 

- Therefore we did not directly use any quality flags other than range flags in this study

- We come up with our own labeled data set focusing on technical anomalies in sensor equipment (Manual labelling process)

- We used interactive graphics which allows the operator to interact with the graphical information presented on the display.

- Most practitioners won't have to do this.

- Purpose of the labelling process: To evaluate the performance of our proposed algorithms.
